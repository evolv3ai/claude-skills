...
Prerequisites:
- Provider skill installed (e.g., `admin-devops (oci)`)
- `admin-devops (coolify)` installed

Steps:
1. Provision server via the provider skill (2+ vCPU, 8GB+ RAM).
2. Add server to inventory with `ROLE=coolify`.
3. Install Coolify using `admin-devops (coolify)`.
4. Configure a tunnel if needed (see `admin-devops (coolify)` references).
5. Update inventory:
...
3. Install services via `admin-devops (app reference)` skills.
