...
- `admin-devops (provider reference)` for provider-specific VM setup.
...
```bash
curl -fsSL https://raw.githubusercontent.com/YOUR-ORG/vibeskills-demo/main/.claude/skills/admin-devops/scripts/coolify-enhanced-setup.sh | bash
```
...
- [Cloudflare Tunnel Setup Script](.claude/skills/admin-devops (coolify)/scripts/coolify-cloudflare-tunnel-setup.sh)
- [Coolify Installation Guide](.claude/skills/admin-devops (coolify)/SKILL.md)
- [OCI Infrastructure Details](.claude/skills/admin-devops (oci)/scripts/.env)
