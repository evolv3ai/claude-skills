...
`admin-devops` stores provider guidance as reference files in `references/`.

Bash:

```bash
refs=(~/.claude/skills/admin-devops/references/*.md)
...
```powershell
$refs = Get-ChildItem (Join-Path $env:USERPROFILE '.claude/skills/admin-devops/references') -Filter *.md
