...
```bash
ls ~/.claude/skills/admin-devops/references/*.md
# If missing, reinstall or resync admin-devops
```

PowerShell:

```powershell
$refsPath = Join-Path $env:USERPROFILE '.claude/skills/admin-devops/references'
